...
const CandlestickChart = ({route}) => {
  ...
  const [btcprice, setBtcPrice] = useState(0); // Default frequency
  ...
  const UpdateWallet = async () => {
    const calculatedPnl = parseFloat(totalBalance) + parseFloat(calculatePnL(currentBalance, calculateROE(entrypoint, currentPrice, currentBalance)))
    const totalBal = (totalBalance*btcprice)+ calculatedPnl
    const InBtcCal =  Number(totalBal)/Number(btcprice)
    try {
      const response = await axios.post(`${api_url}/user/updateWallet`, {
        id: walletId, amount: InBtcCal, name: 'BTC'
      });
      if (response?.data?.status === 200) {
        getWallet()
        setEntryPoint(0)
  ...
  useEffect(() => {
    const getBTCPrice = async() => {
      ...
      setBtcPrice(Number(fromBtc.data?.USD))
    }
    getBTCPrice()
  },[])
  ...
  const selectedAmount = (percentage / 100) * currentBalance * btcprice;
  return <View >
    <Modal
      animationType="slide"
      transparent={true}
      ...
      (tab === "position" && entrypoint > 0) && <View style={styles.positionContainer}>
        ...
        <View style={styles.PRSection}>
          <View style={styles.pnlContainer}>
            ...
            <Text style={[styles.PnlPrice, { color: entrypoint > 0 ? calculatePnL(currentBalance, calculateROE(entrypoint, currentPrice, currentBalance)) > 0 ? '#24b54b' : '#f7584d' : 'black' }]}>{entrypoint > 0 ? formatAmountWithCommas(Number(parseFloat(calculatePnL(currentBalance, calculateROE(entrypoint, currentPrice, currentBalance))))) : '0.0'}</Text>
          </View>
        ...
        <View style={styles.SMRSection}>
          <View style={styles.pnlContainer}>
            <Text style={[styles.pnlTitle, { borderBottomWidth: 1, borderBottomColor: "#b0b3b5", borderStyle: 'dotted' }]}>Size (WIF)</Text>
