...
import candlesticksData from '../data/candledata.json'
...
const CandlestickChart = () => {
  ...
  const [totalBalance, setTotalBalance] = useState(60453.67); // Default frequency
  ...
  const [currentBalance, setCurrentBalance] = useState(60453.67); // Default frequency
  ...
  const getCustomData = async () => {
    ...
    // try {
    //   const response = await axios.get(`${api_url}/data/custom_data`, {
    //   });
    //   setcandleSticksData(candlesticksdata)
    //   // setCurrentBalance(response.data[0]?.totalBalance)
    //   // setTotalBalance(response.data[0]?.totalBalance)
    //   // setcandleSticksData(response.data[0]?.candlesticks)
    // } catch (error) {
